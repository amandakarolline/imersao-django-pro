...
def teste_status_code(resposta_com_tarefa_pendente):
    assert resposta_com_tarefa_pendente.status_code == 302


def teste_tarefa_feita(resposta_com_tarefa_pendente):
    assert Tarefa.objects.get().feita
...
def teste_tarefa_pendente(resposta_com_tarefa_feita):
    assert not Tarefa.objects.get().feita
